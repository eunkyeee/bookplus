...
public class PageCriteria {
	private int pageNumPerPage;  //한페이지에 보여줄 페이지수(상수10)
	...
	private void init() {
		...
		//2) startPage 계산: 한페이지의 종료페이지-한페이지에 보여줄 페이지수+1
		this.startPage=this.endPage-this.pageNumPerPage+1;
		//3) finalEndPage 계산: 올림(전체레코드수/한페이지에 보여줄 페이지 수)
		...
		//5) next 계산: 현재페이지의 종료페이지 * 한페이지에 보여줄 레코드수가 전체 레코드 수보다 작으면 다음 버튼 보이기 
